...
$route_clinical_document_id = get_choice_id_by_name($doc_route_field_id, 'Регламенты и документы подразделения');
$route_patient_route_id = get_choice_id_by_name($doc_route_field_id, 'Служебная записка / приказ / локальный акт');
...
$doc_patient_route_id = ensure_demo_item(
    25,
    242,
    'Служебная записка подразделения: Иван Иванов',
    array_merge([
        'created_by' => 3,
        ...
        'field_251' => '',
        'field_252' => '',
        'field_253' => '<p>Простой внутренний документ подразделения: служебная записка оформляется в рабочем контуре, редактируется в ONLYOFFICE и получает официальный объект в NauDoc.</p><p>Исполнитель: Иван Иванов.</p>',
       ...
       optional_choice_payload_by_name(25, 'Модуль Workspace', 'Calendar')),
    ['Служебная записка подразделения Иван Иванов', 'Направление пациента: Иван Иванов', 'Направление пациента Иван Иванов']
);

$doc_clinical_note_id = ensure_demo_item(
    25,
    242,
    'Регламент подразделения: Иван Иванов',
    array_merge([
        'created_by' => 7,
        'date_added' => demo_timestamp('2026-03-25'),
        'date_updated' => time(),
        ...
        'field_251' => '',
        'field_252' => '',
        'field_253' => '<p>Простой нормативный документ для теста внутреннего маршрута: подготовка регламента подразделения, совместная правка и публикация в официальный контур.</p><p>Ответственный сотрудник: Иван Иванов.</p>',
    ], optional_field_payload($doc_route_field_id, $route_clinical_document_id),
       optional_choice_payload_by_name(25, 'Сценарий DocSpace', 'Collaboration room'),
       optional_choice_payload_by_name(25, 'Модуль Workspace', 'Calendar')),
    ['Регламент подразделения Иван Иванов', 'Медицинская запись отделения: Иван Иванов', 'Медицинская запись отделения Иван Иванов']
);
...
$doc_base_simple_template_id = ensure_demo_item(
    ...
    array_merge([
        ...
        'field_263' => '<p>Обычный рабочий шаблон для быстрой подготовки документа. Можно открыть, скопировать как основу, заполнить данными подразделения или локального регламента и передать в согласование.</p><p>Пример исполнителя: Иван Иванов.</p>',
    ], optional_choice_payload_by_name(26, 'Сценарий DocSpace', 'Public room'),
       optional_choice_payload_by_name(26, 'Модуль Workspace', 'Community')),
...
$doc_base_patient_template_id = ensure_demo_item(
    26,
    255,
    'Шаблон служебной записки подразделения',
    array_merge([
        'created_by' => 3,
        'date_added' => demo_timestamp('2026-03-25'),
        'date_updated' => time(),
        ...
        'field_260' => demo_timestamp('2026-12-31'),
        'field_261' => $naudoc_public_url,
        'field_262' => 'служебная записка,подразделение,согласование',
        'field_263' => '<p>Простой шаблон служебной записки подразделения. Используется как основа для теста внутреннего сценария: заполнить, согласовать и передать в официальный контур.</p>',
    ], optional_choice_payload_by_name(26, 'Сценарий DocSpace', 'Form filling room'),
       optional_choice_payload_by_name(26, 'Модуль Workspace', 'Community')),
    ['Шаблон служебной записки подразделения Иван Иванов', 'Шаблон направления пациента']
);

$doc_base_clinical_template_id = ensure_demo_item(
    26,
    255,
    'Шаблон регламента подразделения',
    array_merge([
        'created_by' => 7,
        'date_added' => demo_timestamp('2026-03-25'),
        'date_updated' => time(),
        ...
        'field_260' => demo_timestamp('2026-12-31'),
        'field_261' => $naudoc_public_url,
        'field_262' => 'регламент,подразделение,нормативный документ',
        'field_263' => '<p>Шаблон внутреннего регламента подразделения. Подходит для быстрого теста ввода данных, редактирования и хранения нормативной служебной документации.</p>',
    ], optional_choice_payload_by_name(26, 'Сценарий DocSpace', 'Collaboration room'),
       optional_choice_payload_by_name(26, 'Модуль Workspace', 'Community')),
    ['Шаблон регламента', 'Шаблон медицинской записи отделения']
);
