...
function entity_field_has_values($entity_id, $field_id)
{
    $query = db_query(
        "select id from app_entity_" . (int) $entity_id .
        " where coalesce(field_" . (int) $field_id . ",'')<>'' limit 1"
    );

    return db_num_rows($query) > 0;
}

function delete_custom_field($entity_id, $field_id)
{
    $field = fetch_row_by_sql(
        "select * from app_fields where entities_id='" . (int) $entity_id . "' and id='" . (int) $field_id . "' limit 1"
    );
    ...
    db_delete_row('app_fields', $field_id);
    db_delete_row('app_fields_choices', $field_id, 'fields_id');
    db_delete_row('app_reports_filters', $field_id, 'fields_id');
    db_query("delete from app_reports_filters_templates where fields_id='" . db_input($field_id) . "'");
    db_query("delete from app_forms_fields_rules where fields_id='" . db_input($field_id) . "'");
    db_query("delete from app_approved_items where entities_id='" . (int) $entity_id . "' and fields_id='" . (int) $field_id . "'");
    db_query("delete from app_access_rules where fields_id='" . db_input($field_id) . "'");
    db_query("delete from app_access_rules_fields where fields_id='" . db_input($field_id) . "'");
    db_query("delete from app_fields_access where entities_id='" . (int) $entity_id . "' and fields_id='" . (int) $field_id . "'");
    db_query("delete from app_onlyoffice_files where field_id='" . (int) $field_id . "'");
    db_delete_row('app_listing_highlight_rules', $field_id, 'fields_id');

    if (is_ext_installed())
    {
        db_delete_row('app_ext_processes_actions_fields', $field_id, 'fields_id');
    }

    choices_values::delete_by_field_id($entity_id, $field_id);
    mind_map::delete_by_fields_id($entity_id, $field_id);
    image_map_nested::delete_by_fields_id($field_id);
    entities::delete_field($entity_id, $field_id);

    return true;
}

function delete_unused_duplicate_onlyoffice_fields($entity_id, $keep_field_id)
{
    $fields_query = db_query(
        "select id from app_fields where entities_id='" . (int) $entity_id . "' and type='fieldtype_onlyoffice' and id!='" . (int) $keep_field_id . "' order by id"
    );
    ...
    while ($field = db_fetch_array($fields_query))
    {
        ...
        if (db_count('app_onlyoffice_files', $field_id, 'field_id') > 0)
        {
            continue;
        }

        if (entity_field_has_values($entity_id, $field_id))
        {
            continue;
        }

        delete_custom_field($entity_id, $field_id);
    }
}
...
function sync_field_choices($entity_id, $field_id, $choices)
{
    ...
    foreach ($choices as $choice)
    {
        ...
        {
            $matched_existing = $existing[$choice['name']];
        }
        elseif (!empty($choice['legacy_names']) && is_array($choice['legacy_names']))
        {
            foreach ($choice['legacy_names'] as $legacy_name)
            {
                if (isset($existing[$legacy_name]))
                {
                    $matched_existing = $existing[$legacy_name];
                    break;
                }
            }
        }
        ...
        if ($matched_existing)
        {
            db_perform('app_fields_choices', $data, 'update', "id='" . db_input($matched_existing['id']) . "'");
        }
        else
        {
...
function normalize_choice_id_csv($value, $from_id, $to_id)
{
    $from_id = (int) $from_id;
    $to_id = (int) $to_id;
    ...
    foreach ($parts as $part)
    {
        ...
        if ($choice_id === $from_id)
        {
            $choice_id = $to_id;
        }

        if ($choice_id <= 0 || in_array($choice_id, $normalized, true))
        {
            continue;
        }
...
function merge_choice_id_references_in_filter_table($table, $field_id, $from_id, $to_id)
{
    $field_id = (int) $field_id;
    $query = db_query(
        "select id, filters_values from " . $table . " where fields_id='" . $field_id . "'"
    );

    while ($row = db_fetch_array($query))
    {
        $updated = normalize_choice_id_csv($row['filters_values'], $from_id, $to_id);

        if ($updated === (string) $row['filters_values'])
        {
            continue;
        }

        db_perform($table, [
            'filters_values' => $updated,
        ], 'update', "id='" . db_input($row['id']) . "'");
    }
}

function merge_choice_id_references($field_id, $from_id, $to_id)
{
    $field_id = (int) $field_id;
    $from_id = (int) $from_id;
    $to_id = (int) $to_id;

    if ($field_id <= 0 || $from_id <= 0 || $to_id <= 0 || $from_id === $to_id)
    {
        return;
    }

    $field = fetch_row_by_sql("select entities_id from app_fields where id='" . $field_id . "' limit 1");

    if ($field && (int) $field['entities_id'] > 0)
    {
        $entity_id = (int) $field['entities_id'];

        db_query(
            "update app_entity_" . $entity_id .
            " set field_" . $field_id . "='" . $to_id . "'" .
            " where field_" . $field_id . "='" . $from_id . "'"
        );

        $values_table = fetch_row_by_sql("show tables like 'app_entity_" . $entity_id . "_values'");
        if ($values_table)
        {
            db_query(
                "update app_entity_" . $entity_id . "_values" .
                " set value='" . $to_id . "'" .
                " where fields_id='" . $field_id . "' and value='" . $from_id . "'"
            );
            db_query(
                "delete duplicate_rows from app_entity_" . $entity_id . "_values duplicate_rows" .
                " inner join app_entity_" . $entity_id . "_values kept_rows" .
                " on duplicate_rows.id > kept_rows.id" .
                " and duplicate_rows.items_id = kept_rows.items_id" .
                " and duplicate_rows.fields_id = kept_rows.fields_id" .
                " and duplicate_rows.value = kept_rows.value" .
                " where duplicate_rows.fields_id='" . $field_id . "'"
            );
        }
    }

    merge_choice_id_references_in_filter_table('app_reports_filters', $field_id, $from_id, $to_id);
    merge_choice_id_references_in_filter_table('app_reports_filters_templates', $field_id, $from_id, $to_id);
    merge_choice_id_references_in_filter_table('app_user_filters_values', $field_id, $from_id, $to_id);
}

function ensure_choice_legacy_rename($field_id, $legacy_name, $target_name)
{
    $field_id = (int) $field_id;
    $legacy_name = trim((string) $legacy_name);
    $target_name = trim((string) $target_name);

    if ($field_id <= 0 || !strlen($legacy_name) || !strlen($target_name) || $legacy_name === $target_name)
    {
        return;
    }

    $legacy = fetch_row_by_sql(
        "select * from app_fields_choices where fields_id='" . $field_id . "' and name='" . db_input($legacy_name) . "' order by id limit 1"
    );
    ...
    $target = fetch_row_by_sql(
        "select * from app_fields_choices where fields_id='" . $field_id . "' and name='" . db_input($target_name) . "' order by id limit 1"
    );

    if ($target && (int) $target['id'] !== (int) $legacy['id'])
    {
        merge_choice_id_references($field_id, (int) $target['id'], (int) $legacy['id']);

        db_perform('app_fields_choices', [
        ...
        ], 'update', "id='" . db_input($legacy['id']) . "'");

        db_query("delete from app_fields_choices where id='" . db_input($target['id']) . "'");
        return;
    }
...
function ensure_common_report($entity_id, $name, $params = [])
{
    ...
    )
    {
        ...
        {
            $legacy_name = trim((string) $legacy_name);
            if (!strlen($legacy_name))
            {
                continue;
            }

            $report = fetch_row_by_sql(
                "select * from app_reports where entities_id='" . (int) $entity_id . "' and reports_type='common' and name='" . db_input($legacy_name) . "' limit 1"
            );
...
ensure_entity_record(21, 0, $operations_group_id, 'Проекты и инициативы', 10, 1, 'Клинические, административные, ИТ и хозяйственные проекты учреждения.');
ensure_entity_record(22, 21, $operations_group_id, 'Поручения и задачи', 11, 1, 'Задачи и поручения внутри проектов и процессов.');
ensure_entity_record(23, 0, $operations_group_id, 'Заявки на обслуживание', 20, 1, 'Сервисные и документные заявки от сотрудников, врачей и регистратуры.');
...
$project_team_tab = ensure_form_tab(21, 'Ответственные и роли', 1, ['Команда и роли', 'Команда']);
...
$project_curator_id = ensure_custom_field(21, 'Заведующий / куратор подразделения', 'fieldtype_users', $project_team_tab, [
    'configuration' => config_json(['display_as' => 'dropdown', 'allow_search' => '1']),
    'sort_order' => 2,
    'listing_status' => 1,
...
$project_docspace_link_id = ensure_custom_field(21, 'Комната DocSpace', 'fieldtype_input_url', $project_control_tab, [
...
sync_field_choices(21, $project_department_id, [
    ...
    ['name' => 'Поликлиника', 'sort_order' => 2, 'bg_color' => '#bee1e6', 'legacy_names' => ['Учебный отдел']],
    ['name' => 'Стационар', 'sort_order' => 3, 'bg_color' => '#cdb4db', 'legacy_names' => ['Кафедра / преподаватели']],
    ['name' => 'Приемное отделение', 'sort_order' => 4, 'bg_color' => '#ffc8dd', 'legacy_names' => ['Методический отдел']],
    ['name' => 'ИТ и хозслужба', 'sort_order' => 5, 'bg_color' => '#bde0fe', 'legacy_names' => ['ИТ и сервис']],
    ['name' => 'Хозяйственная служба / снабжение', 'sort_order' => 6, 'bg_color' => '#ffe5b4', 'legacy_names' => ['АХО / снабжение']],
    ['name' => 'Регистратура', 'sort_order' => 7, 'bg_color' => '#d8f3dc'],
    ['name' => 'Канцелярия', 'sort_order' => 8, 'bg_color' => '#f8edeb'],
]);
...
$request_group_id = ensure_custom_field(23, 'Инициатор / подразделение', 'fieldtype_grouped_users', $request_main_tab, [
    'is_required' => 1,
    'sort_order' => 0,
    'listing_status' => 1,
    'listing_sort_order' => 4,
...
$request_docspace_link_id = ensure_custom_field(23, 'Комната DocSpace', 'fieldtype_input_url', $request_execution_tab, [
    'configuration' => config_json(['width' => 'input-xlarge', 'target' => '_blank', 'preview_text' => 'Открыть комнату']),
    'sort_order' => 7,
]);
...
sync_field_choices(23, $request_type_id, [
    ['name' => 'Нормативный документ', 'sort_order' => 1, 'is_default' => 1, 'bg_color' => '#bde0fe', 'legacy_names' => ['Методический вопрос', 'Медицинская документация']],
    ['name' => 'Административный документ', 'sort_order' => 2, 'bg_color' => '#cdb4db', 'legacy_names' => ['Документооборот']],
...
sync_field_choices(23, $request_group_id, [
    ...
    ['name' => 'Поликлиника', 'sort_order' => 2, 'bg_color' => '#bee1e6', 'users' => '1', 'legacy_names' => ['Учебный отдел']],
    ['name' => 'Стационар', 'sort_order' => 3, 'bg_color' => '#cdb4db', 'users' => '1', 'legacy_names' => ['Кафедра / преподаватели']],
    ['name' => 'Приемное отделение', 'sort_order' => 4, 'bg_color' => '#ffc8dd', 'users' => '1', 'legacy_names' => ['Методический отдел']],
    ['name' => 'ИТ и хозслужба', 'sort_order' => 5, 'bg_color' => '#bde0fe', 'users' => '1', 'legacy_names' => ['ИТ и сервис']],
    ['name' => 'Хозяйственная служба / снабжение', 'sort_order' => 6, 'bg_color' => '#ffe5b4', 'users' => '1', 'legacy_names' => ['АХО / снабжение']],
    ['name' => 'Регистратура', 'sort_order' => 7, 'bg_color' => '#d8f3dc', 'users' => '1'],
    ['name' => 'Канцелярия', 'sort_order' => 8, 'bg_color' => '#f8edeb', 'users' => '1'],
]);
...
sync_field_choices(23, $request_service_category_id, [
    ['name' => 'Документы и регистрация', 'sort_order' => 1, 'is_default' => 1, 'legacy_names' => ['Документы']],
    ['name' => 'Нормативные документы', 'sort_order' => 2, 'legacy_names' => ['Учебный процесс', 'Медицинские процессы']],
    ['name' => 'ИТ и доступ', 'sort_order' => 3, 'legacy_names' => ['ИТ']],
    ['name' => 'АХО / МТЗ', 'sort_order' => 4],
    ['name' => 'Административное обслуживание', 'sort_order' => 5],
]);
ensure_choice_legacy_rename($request_service_category_id, 'Медицинские процессы', 'Нормативные документы');
...
$doc_route_id = ensure_custom_field(25, 'Маршрут документа', 'fieldtype_dropdown', $doc_flow_tab, [
    'is_required' => 1,
    'configuration' => config_json(['width' => 'input-large']),
    'sort_order' => 1,
    'listing_status' => 1,
...
$doc_draft_id = ensure_custom_field(25, 'Совместное редактирование', 'fieldtype_onlyoffice', $doc_card_tab, [
...
$doc_docspace_link_id = ensure_custom_field(25, 'Комната DocSpace', 'fieldtype_input_url', $doc_flow_tab, [
    'configuration' => config_json(['width' => 'input-xlarge', 'target' => '_blank', 'preview_text' => 'Открыть комнату']),
    'sort_order' => 4,
]);
...
delete_unused_duplicate_onlyoffice_fields(25, $doc_draft_id);
...
sync_field_choices(25, $doc_type_id, [
    ...
    ['name' => 'Нормативный документ', 'sort_order' => 4, 'bg_color' => '#f8edeb', 'legacy_names' => ['Медицинский документ']],
    ['name' => 'Служебная записка', 'sort_order' => 5, 'bg_color' => '#ffd6a5'],
    ['name' => 'Приказ', 'sort_order' => 6, 'bg_color' => '#cce3de'],
    ['name' => 'Договор', 'sort_order' => 7, 'bg_color' => '#cdb4db'],
    ['name' => 'Акт', 'sort_order' => 8, 'bg_color' => '#ffe5b4'],
...
ensure_choice_legacy_rename($doc_type_id, 'Медицинский документ', 'Нормативный документ');
...
sync_field_choices(25, $doc_route_id, [
    ...
    ['name' => 'Регламенты и документы подразделения', 'sort_order' => 4, 'bg_color' => '#f8edeb', 'legacy_names' => ['Маршрут медицинского документа', 'Медицинская документация отделения']],
    ['name' => 'Служебная записка / приказ / локальный акт', 'sort_order' => 5, 'bg_color' => '#ffd6a5', 'legacy_names' => ['Маршрут пациента / направления', 'Пациент / направление / выписка']],
...
ensure_choice_legacy_rename($doc_route_id, 'Медицинская документация отделения', 'Регламенты и документы подразделения');
ensure_choice_legacy_rename($doc_route_id, 'Пациент / направление / выписка', 'Служебная записка / приказ / локальный акт');
...
$doc_base_docspace_link_id = ensure_custom_field(26, 'Комната DocSpace', 'fieldtype_input_url', $doc_base_review_tab, [
    'configuration' => config_json(['width' => 'input-xlarge', 'target' => '_blank', 'preview_text' => 'Открыть комнату']),
    'sort_order' => 5,
]);
...
$mts_docspace_link_id = ensure_custom_field(27, 'Комната DocSpace', 'fieldtype_input_url', $mts_supply_tab, [
    'configuration' => config_json(['width' => 'input-xlarge', 'target' => '_blank', 'preview_text' => 'Открыть комнату']),
    'sort_order' => 5,
]);
...
$patient_clinical_report = ensure_common_report(25, 'Нормативные и внутренние документы', [
    'description' => 'Боевой маршрут первой волны для регламентов, служебных записок, локальных актов и внутренних документов подразделения.',
    'legacy_names' => ['Пациентские и клинические документы'],
...
sync_report_filters($patient_clinical_report, [
    ['fields_id' => $doc_route_id, 'values' => array_filter([
        get_choice_id_by_name($doc_route_id, 'Служебная записка / приказ / локальный акт'),
        get_choice_id_by_name($doc_route_id, 'Регламенты и документы подразделения'),
    ])],
    ['fields_id' => $doc_status_id, 'values' => array_filter([
        get_choice_id_by_name($doc_status_id, 'Черновик'),
        get_choice_id_by_name($doc_status_id, 'На согласовании'),
...
foreach ([$manager_group_id, $employee_group_id, $nurse_group_id, $requester_group_id, $office_group_id] as $restricted_group_id)
{
    sync_fields_access(21, $restricted_group_id, [
        ...
        $project_workspace_link_id => 'hide',
    ]);
    sync_fields_access(23, $restricted_group_id, [
        $request_docspace_link_id => 'hide',
        $request_workspace_link_id => 'hide',
    ]);
    sync_fields_access(25, $restricted_group_id, [
        $doc_docspace_link_id => 'hide',
        $doc_workspace_link_id => 'hide',
    ]);
    sync_fields_access(26, $restricted_group_id, [
        $doc_base_docspace_link_id => 'hide',
        $doc_base_workspace_link_id => 'hide',
    ]);
    sync_fields_access(27, $restricted_group_id, [
        $mts_docspace_link_id => 'hide',
        $mts_workspace_link_id => 'hide',
    ]);
}
...
db_query("delete from app_entities_menu where parent_id='0' and name='Главная'");
ensure_entities_menu_item('Работа', [
    'icon' => 'fa-tasks',
    'icon_color' => '#1f4f86',
    'bg_color' => '#e8f1fb',
...
ensure_entities_menu_item('Контроль', [
    'icon' => 'fa-line-chart',
    'icon_color' => '#204b76',
    'bg_color' => '#d9e8f6',
    'reports_list' => [
