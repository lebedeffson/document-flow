...
{
    if(isset($import_fields[$col]) and isset($worksheet[$row][$col]))
    {
        ...
        if($filed_info = db_fetch_array($filed_info_query))
        {
            ...
            {
                $check_query = db_query("select id from app_entity_{$entities_id} where field_{$field_id}='" . db_input($sql_data['field_' . $field_id]) . "' " . ($cfg->get('is_unique')==2 ? "and parent_item_id = '" . $import_entity_parent_item_id . "'":""). " limit 1");
                if($check = db_fetch_array($check_query))
                {
