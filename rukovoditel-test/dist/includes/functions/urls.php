...
function get_request_host()
{
    ...
    if (strpos($forwarded_port, ',') !== false)
    {
        $forwarded_port = trim(explode(',', $forwarded_port)[0]);
    }

    if (strlen($forwarded_port) && strpos($host, ':') === false)
    {
        $is_default_port = (is_ssl() && $forwarded_port === '443') || (!is_ssl() && $forwarded_port === '80');
        if (!$is_default_port)
        {
            $host .= ':' . $forwarded_port;
        }
...
function is_ssl()
{
    return (ENABLE_SSL || IS_HTTPS) ? true : false;
}
