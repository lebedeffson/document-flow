...
class file_storage_field
{
    ...
    static function preview($entity_id, $field_id, $token, $item_id)
    {
        ...
        $field_query = db_query("select id, name, configuration,type, is_required from app_fields where id='" . $field_id . "'");
        ...
        {          
            ...
            {
                return input_hidden_tag('fields[' . $field_id . ']', '', array('class' => 'field_' . $field['id'] . ' required'));
            }
            else
            {
                return '';
