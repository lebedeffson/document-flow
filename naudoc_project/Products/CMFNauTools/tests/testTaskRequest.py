...
$Id: testTaskRequest.py,v 1.2 2006/02/22 11:34:13 ynovokreschenov Exp $
"""
...
class TaskRequestTests(NauDocTestCase.NauFunctionalTestCase):
    ...
    def testTaskRequestSatisfy(self):
        ...
        # test followup/task_001/task_response_form?status=satisfy
        path = '/%s/task_response_form?status=satisfy' % task.getObject().absolute_url(1)
        response = self.publish(path, user_auth )
        self.assertResponse( response )
    ...
    def testTaskRequestReject(self):
        users = self.naudoc.acl_users.getUsers()
        portal_followup = getToolByName( self.naudoc, 'portal_followup' )
        storage = self.naudoc.storage.members
        doc = storage._getOb( self.test_document )
...
def test_suite():
    ...
    suite = TestSuite()
    suite.addTest( makeSuite(TaskRequestTests) )
    return suite
