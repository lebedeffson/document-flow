...
# Embedded file name: /home/naudoc/DistributiveMonster/build/10.03.2009_19-24/Products/CMFNauTools/Addons/GenericTaskTypes/tests/testModificationTask.py
...
class ModificationTaskFunctionalTests(NauDocTestCase.NauDocTestCase):
    __module__ = __name__
    _remove_naudoc = 0
    ...
    def afterSetUp(self):
        ...
        self.modification = self.task_container.getTask(self.task_container.createTask(title, type=type, bind_to=None, creator=None, REQUEST=None, **kw))
    ...
    def testOnFinalize(self):
        ...
        result_code = 'success'
        modification.onFinalize(REQUEST=REQUEST, result_code=result_code)
        self.assert_(modification.isFinalized())
        return

    def testOnCommit(self):
        modification = self.modification
        REQUEST = self.app.REQUEST
        modification.onCommit(REQUEST=REQUEST)
        return

    def testOnReject(self):
        modification = self.modification
        REQUEST = self.app.REQUEST
        modification.onReject(REQUEST=REQUEST)
        return

    def testOnCancel(self):
        modification = self.modification
        REQUEST = self.app.REQUEST
        modification.onCancel(REQUEST=REQUEST)
        self.assertEqual(modification.result, 'cancelled')
        return

    def beforeTearDown(self):
        self.modification.deleteObject()
...
def test_suite():
    ...
    suite = TestSuite()
    suite.addTest(makeSuite(ModificationTaskFunctionalTests))
    return suite
    return
