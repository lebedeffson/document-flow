...
class LocalPropertyManager(LanguageManager, LocalAttributesBase):
    ...
    def _setLocalProperty(self, id, type, lang=None, value=None):
        ...
        if not self.hasLocalProperty(id):
            self._local_properties_metadata = self._local_properties_metadata \
                                              + ({'id': id, 'type': type},)
    ...
    def get_available_languages(self, **kw):
        ...
        if id is None:
            ...
            property = self._local_properties[id]
            return [ x for x in self._languages if property.get(x, None) ]


    security.declarePublic('get_default_language')
    def get_default_language(self):
        """ """
        return self._default_language or self._languages[0]
    ...
    def get_path_for_cookies(self):
        ...
        url_self = self.absolute_url()         # Url to this object
        url_referer = request['HTTP_REFERER']  # Url we come from
        if len(url_self) < len(url_referer):   # Choose the shorter url
            path = url_self
        else:
            path = url_referer
