...
$Id: Stopwords.py,v 1.1.1.1 2004/09/20 12:10:01 vpastukhov Exp $
"""


import os, re
from types import DictType, StringType, DictType, ListType, TupleType
...
class Stopwords:
    ...
    def __init__(self, language, words):
        ...
        if isinstance(words, DictType): self._words.update(words)
        elif isinstance(words, TupleType) or isinstance(words,ListType):
...
class FileStopwords(Stopwords):
    ...
    def readStopWords(self, filename):
        ...
        try:        
            f = os.path.join(_dir,'stopwords',filename) 
