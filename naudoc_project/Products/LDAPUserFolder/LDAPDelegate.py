...
class LDAPDelegate(Persistent):
    ...
    def search( self
        ...
        try:
            ...
            for base_dn in base:
                try:
                    res = connection.search_s(base_dn, scope, filter, attrs)
                except ldap.NO_SUCH_OBJECT:
                    not_found.append(base_dn)
            ...
            if not_found:
                result['exception'] = 'Cannot find %s under %s' % (filter, '; '.join(not_found))
    ...
    def modify(self, dn, mod_type=None, attrs=None):
        ...
        res = self.search(base=to_utf8(dn), scope=BASE)
        attrs = attrs and attrs or {}
        ...
        try:
            connection = self.connect()
            connection.modify_s(to_utf8(dn), mod_list)
        except ldap.INVALID_CREDENTIALS, e:
            e_name = e.__class__.__name__
        ...
        if new_rdn and new_rdn != cur_rec.get(rdn_attr):
            ...
            try:
                connection.modrdn_s(to_utf8(dn), to_utf8(new_dn), 0)
            except ldap.INVALID_CREDENTIALS, e:
                e_name = e.__class__.__name__
                msg = '%s No permission to modify "%s"' % (e_name, dn)
            except ldap.REFERRAL, e:
