...
#
##############################################################################
"""$Id: zopeedit.py,v 1.3 2004/06/16 09:41:00 ypetrov Exp $
"""
...
_ = translation('messages', os.path.join(sys.path[0], 'locale'), languages = ['ru']).gettext
...
class ExternalEditor:
    ...
    def __init__(self, input_file):
        try:
            ...
            if win32:
                ...
                if profile_path:
                    config_path = os.path.join( profile_path, 'ZopeEdit.ini' )
                    if not os.path.exists( config_path ) and os.path.exists( global_config ):
            ...
            # XXX must remove special characters [$%"\/:] from the filename
            #content_file = metadata.get('filename') or os.path.split(self.path)[1]

            content_file = self.path.split('/')[-1]
            extension = self.options.get('extension')

            if not extension or content_file.lower().endswith(extension.lower()):
                content_file, extension = os.path.splitext(content_file)

            if self.options.has_key('temp_dir'):
                tempdir = os.path.expanduser(self.options['temp_dir'])
            else:
                tempdir = gettempdir()

            tempname = os.path.join(tempdir, content_file + extension)
            count = 1
            while os.path.exists(tempname):
                tempname = os.path.join(tempdir, content_file + ('[%d]' % count) + extension)
                count += 1
            content_file = tempname
            ...
            if self.ssl:
                ...
                try:
                    ...
                    fatalError(_('SSL support is not available on this system. '
    ...
    def getEditorCommand(self):
        ...
        if win32 and editor is None:
            ...
            if editor is not None:
                # Convert command string from Unicode
                editor = str(editor)
                # Don't use IE as an "editor"
                if editor.lower().find('\\iexplore.exe') != -1:
                    editor = None

        if not editor and not win32 and has_tk():
            from tkSimpleDialog import askstring
            editor = askstring(_('Zope External Editor'),
                               _('Enter the command to launch the default editor'))
            if not editor:
                sys.exit(0)
    ...
    def launch(self):
        ...
        if use_locks and self.metadata.get('lock-token'):
            ...
            if (int(self.options.get('always_borrow_locks', 0))
                ...
                or askYesNo(_('This object is already locked by you in another'
                              ' session.\n'
        ...
        if bin is None:
            ...
            if command.find(file_insert) >= 0:
                command = command.replace(file_insert, self.content_file)
            else:
                command += ' "%s"' % self.content_file
        ...
        while retries_count > 0:
            ...
            is_alive = editor.isAlive()

            if save_interval or not is_alive:
        ...
        if not self.saved \
           and askYesNo(_('File not saved to Zope.\nReopen local copy?')):
            self.launch()
    ...
    def putChanges(self):
        ...
        if int(self.options.get('use_locks', 0)) and self.lock_token is None:
            ...
            if not self.lock():
                ...
                if not askYesNo(_('Could not acquire lock. '
    ...
    def lock(self):
        ...
        if response.status / 100 == 2:
            ...
            if response.status == 423:
                message = _('(object already locked)')
            else:
                message = ''

            if self.askRetryAfterError(response, _('Lock request failed'),
                                       message):
                self.lock()
            else:
                self.did_lock = 0
    ...
    def unlock(self, interactive=1):
        ...
        if interactive and response.status / 100 != 2:
            ...
            if self.askRetryAfterError(response, _('Unlock request failed')):
                self.unlock()
            else:
                self.did_lock = 0
        else:
    ...
    class EditorProcess:
        def __init__(self, command):
            ...
            try:
                ...
                fatalError(_('Error launching editor process\n'
                             '(%s):\n%s') % (command, e[2]))
...
def fatalError(message, exit=1):
    ...
    errorDialog(_('FATAL ERROR: %s') % message)
...
if __name__ == '__main__':
    try:
        ...
        if '--version' in args or '-v' in args:
            credits = _('Zope External Editor %s\n'
        ...
        fatalError(_('Input file name missing.\n'
