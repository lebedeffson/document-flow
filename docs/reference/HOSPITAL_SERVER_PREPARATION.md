...
5. при использовании `--data-root` туда выносятся persistent-данные платформы, включая `MariaDB`, `Bridge`, `ONLYOFFICE Docs`, служебные хранилища и `NauDoc Data.fs`
6. если включается same-host live office слой, его persistent-данные тоже складываются под этим же корнем, в каталоге `office-live/`
...
8. если указан `--data-root`, выносит persistent-данные платформы, `NauDoc Data.fs` и same-host office persistent state на этот путь
